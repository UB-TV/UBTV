...
import { EditoProgram } from "@/Constants/Temp";
// Component
            ...
            {program && (
                <>
                    ...
                    <section className="flex flex-col gap-3 w-full mt-6">
                        ...
                        <div className="flex gap-6 flex-wrap">
                            {episodes.map((episode, index) => (
                                <div key={index + 1} className="w-full mt-4">
                                    <h2 className="heading-6 font-semibold mb-2">Episode {index + 1} ({episode.segment_count} Segmen)</h2>
                                    <div className="flex gap-6 flex-wrap">
                                        {episode.videos && episode.videos.map((video, index) => (
                                            <SegmentCard key={index} segmentNumber={video.segment_number} thumbnailUrl={'/image/program-thumbnail.jpg'} />
                                        ))}
                                    </div>
                                </div>
                            ))}
                        ...
                        <Dialog size="Normal" toggleDialog={toggleDialog} ref={dialogRef}>
                            <h1 className="heading-2 font-semibold text-left">Upload Video</h1>
                            <UploadSegmentForm episode={episodes} />
                        </Dialog>
                    </section>
                </>
            )}
